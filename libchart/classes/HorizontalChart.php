	...
	class HorizontalChart extends BarChart
	{
		...
		*/
		...
		function printAxis()
		{
			...
			{
				$x = $this->graphTLX + ($value - $minValue) * ($this->graphBRX - $this->graphTLX) / ($maxValue - $minValue);
				...
				$this->text->printText($this->img, $x, $this->graphBRY + 5, $this->textColor, number_format($value), $this->text->fontCondensed, $this->text->HORIZONTAL_CENTER_ALIGN);
			}
		...
		*/
		...
		function printBar()
		{
			...
			for($i = 0; $i < $this->sampleCount; $i++)
			{
				...
				$xmax = $this->graphTLX + ($value - $minValue) * ($this->graphBRX - $this->graphTLX) / ($maxValue - $minValue);
				...
				imagefilledrectangle($this->img, $this->graphTLX + 1, $y1, $xmax, $y2, $this->barColor2);
				imagefilledrectangle($this->img, $this->graphTLX + 2, $y1+1, $xmax - 4, $y2, $this->barColor1);
			}
		}
