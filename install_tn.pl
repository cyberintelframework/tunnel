...
print "${y}Also install the 1.03-package-log part of the SURFnet IDS system.\n${n}";
...
if (! -e "$targetdir/serverkeys/dh${key_size}.pem") {
  ...
  `$targetdir/genkeys/build-dh 2>/dev/null`;
...
`sed 's/^server=\"enter_server_name\"\$/server=\"$server\"/' ./updates/scripts.conf > $targetdir/updates/scripts.conf`;
...
`$targetdir/scriptkeys/sign_all.sh`;
...
print "  $targetdir/updates/scripts.conf${n}\n";
