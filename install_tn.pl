...
#####################
# Changelog:
# 004 Fixed removal of obsolete files
# 003 Added ssl dir existance check
# 002 Setting default installation type to "install"
# 001 version 2.00
#####################
...
if (-e "$targetdir/server/") {
  printmsg("SURFids tunnel server already installed:", "info");
  $itype = "none";
...
printdelay("Copying SURFids files:");
...
if ($itype eq "install") {
  ...
  print VARS "export KEY_UNITNAME=\"SURFids\"\n";
...
if (! -e "$ssldir") {
  printdelay("Creating SSL directory:");
...
print "#####################################\n";
print "# ${g}SURFids installation complete${n} #\n";
print "#####################################\n";
print "\n";
...
print "NOTICE: Also install the logging server part of the SURFids system!\n";
print "\n";
