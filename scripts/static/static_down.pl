...
# Get sensor id
$sth = $dbh->prepare("SELECT sensors.id FROM sensors, tap WHERE tap.sensor_id = sensors.id AND tap.dev = '$if'");
$ts = getts();
print LOG "[$ts - $tap] Prepared query: SELECT sensors.id FROM sensors, tap WHERE tap.sensor_id = sensors.id AND tap.dev = '$if'\n";
...
# Get mac
#$mac = `ifconfig $if | grep HWaddr`;
#@mac_ar = split(/ +/, $mac);
#$mac = $mac_ar[4];
#print LOG "[$ts - $if] Getting mac with ifconfig $if | grep HWaddr. Mac: $mac.\n";

# Get the IP address configuration for the tap device from the database.
$sth = $dbh->prepare("
SELECT static_ip, static_netmask, static_gateway, static_broadcast
FROM   netconfig
...
");
$ts = getts();
print LOG "[$ts - $tap] Prepared query: SELECT static_ip, static_netmask, static_gateway, static_broadcast FROM netconfig WHERE sensor_id = ...\n";
$execute_result = $sth->execute();
$ts = getts();
print LOG "[$ts - $tap] Executed query: $execute_result \n";
...
$ts = getts();
$if_ip = $row[0];
print LOG "[$ts - $tap] Query result: $if_ip (empty=dhcp) \n";
...
if ( ! $remoteip ) {
  ...
  $sth = $dbh->prepare("SELECT sensors.remoteip FROM sensors, tap WHERE tap.sensor_id = sensor.id AND tap.dev = '$if'");
  $ts = getts();
  print LOG "[$ts - $if] Prepared query: SELECT sensors.remoteip FROM sensors, tap WHERE tap.sensor_id = sensor.id AND tap.dev = '$if'\n";
  $execute_result = $sth->execute();
  $ts = getts();
...
# Get mac_id
$sth = $dbh->prepare("SELECT mac_id FROM tap WHERE dev = '$if'");
$ts = getts();
print LOG "[$ts - $if] Prepared query: SELECT mac_id FROM tap WHERE dev = '$if'\n";
$execute_result = $sth->execute();
$ts = getts();
print LOG "[$ts - $if] Executed query: $execute_result\n";
@row = $sth->fetchrow_array;
...
# Set used = FALSE for this mac
$sth = $dbh->prepare("UPDATE mac SET used = 'f' WHERE id = '$mac_id'");
$ts = getts();
print LOG "[$ts - $if] Prepared query: UPDATE mac SET used = 'f' WHERE id = '$mac_id'\n";
$execute_result = $sth->execute();
$ts = getts();
print LOG "[$ts - $if] Executed query: $execute_result\n";

# Set used = FALSE for this sensors netconfig records
$sth = $dbh->prepare("UPDATE netconfig SET used = 'f' WHERE sensor_id = '$sensor_id'");
$ts = getts();
print LOG "[$ts - $if] Prepared query: UPDATE netconfig SET used = 'f' WHERE sensor_id = '$sensor_id'\n";
$execute_result = $sth->execute();
$ts = getts();
print LOG "[$ts - $if] Executed query: $execute_result\n";

# Execute query to remove tap device information from database.
$execute_result = $dbh->do("DELETE FROM tap WHERE dev = '$if' AND sensor_id = '$sensor_id'");
$ts = getts();
print LOG "[$ts - $if] Prepared query: DELETE FROM tap WHERE dev = '$if' AND sensor_id = '$sensor_id'\n";
print LOG "[$ts - $if] Executed query: $execute_result\n";
