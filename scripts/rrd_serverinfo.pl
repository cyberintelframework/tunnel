...
#  print "$_[0] usage use_rate:$use_rate%, available:${avail}B, used:${used}B\n";
...
#  my $memory_avail = $memory[3] * 1024;
#  my $memory_used = $memory[2] * 1024;
  ...
  $swap =~ s/ +/ /g;
  my @swap = split " ", $swap;
#  my $swap_avail = $swap[3] * 1024;
#  my $swap_used = $swap[2] * 1024;
  my $swap_avail = $swap[3];
  my $swap_used = $swap[2];
  my $swap_use_rate = ( $swap_used / ( $swap_used + $swap_avail ) ) * 100;

#  print "memory usage use_rate:$memory_use_rate%, available:${memory_avail} bytes, used:${memory_used} bytes\n";
#  print "swap usage use_rate:$swap_use_rate%, available:${swap_avail} bytes, used:${swap_used} bytes\n";
