...
################
# Changelog:
# 009 Added randommac support for VLAN's
# 008 Fixed mii-tool bug
# 007 Added getpid
...
# 1.20 chksys
# Function to check if the syslog daemon is running or not
# Returns 1 if the syslog daemon is running
# Returns 0 if the syslog daemon is not running
sub chksys() {
  my ($chk);
  $chk = `ps -ef | grep syslogd | grep -v grep | wc -l`;
  chomp($chk);
  if ($chk > 0) {
    return 1;
  } else {
...
sub getif() {
  ...
  while(<DEVNET>) {
    if ($_ =~ /(eth.:|tr.:)/) {
      ...
      @if_ar = split(/:/, $if_ar[1]);
      $if = $if_ar[0];
      $checkunspec = `ifconfig $if | grep UNSPEC | wc -l`;
      if ($checkunspec == 0) {
...
# 3.23 chkuprun 
# Function to check if the up script is running 
# Returns 0 if up is running
# Returns 1 if not 
sub chkuprun() {
  $chkuprun = `ps -ef |grep "perl" |grep "up" |grep -v "update" |grep -v grep |wc -l`;
  chomp($chkuprun);
  if ($chkuprun != 0) {
    return 0;
  } else {
